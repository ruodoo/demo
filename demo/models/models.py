...
class User(models.Model):
    ...
    hobbies = fields.Many2many(
        "demo.hobby",
        string="Hobbies",
        relation="demo_user_hobby",
        column1="user_id",
        column2="hobby_id",
    )
    description = fields.Char(
        string="Description", required=True, default="No description"
    )
    ...
    def _description_is_one_line(self):
        for user in self:
            if user.description and "\n" in user.description:
                raise ValueError(
                    f"Description must be oneline, got `{user.description}`"
                )
