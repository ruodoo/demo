...
class TestHobby(TransactionCase):
    def test_name(self):
        hobby = self.env["demo.hobby"].create({"name": "Programming"})
    ...
    def test_description_one_line(self):
        with self.assertRaises(ValueError) as error_catcher:
            self.env["res.users"].create(
                {"name": "Jeremy", "login": "jem", "description": "I like \n Sports"}
            )
        self.assertEqual(
            error_catcher.exception.args[0], Like("Description must be oneline ...")
        )

    def test_hobby(self):
        minecraft = self.env.ref("demo.hobby_minecraft")
        user = self.env["res.users"].create(
            {"name": "Pierre", "login": "pie", "hobby": minecraft.id}
        )
